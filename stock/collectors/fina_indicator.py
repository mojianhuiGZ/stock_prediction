...
@singleton
class FinaIndicatorCollector(TushareMongodbBaseCollector):
    ...
    def __init__(self,
                 ...
                 database_name=''):
        super().__init__(
            collection_name='fina_indicator',
    ...
    def getFinaIndicator(self, ts_code, start_date=None, end_date=None):
        tushare = self.getTushare()
        data = tushare.fina_indicator(
            ts_code=ts_code,
            ann_date='',
    ...
    def update(self):
        logging.info('update 财务指标数据(fina_indicator) ...')
        ...
        for ts_code in ts_codes:
            ...
            stock_quarters = set()
            for doc in collection.find({'ts_code': ts_code}, {'end_date': 1}):
            ...
            logging.debug('get {}-{} fina_indicator of {} from tushare'.format(
            ...
            while True:
                try:
                    data = self.getFinaIndicator(ts_code, start_date, end_date)
                    break
