...
def getListOfDates(startdate, enddate):
    ...
    dates = [
        ...
        for m in range(startdate.year * 12 + startdate.month -
...
def draw(df, date_column, title="", colorup='g', colordown='r'):
    # 畫出價量曲線

    # 檢查實際讀到的日期
    if not date_column in df.columns:
        return

    startdate = df[date_column][0]
    enddate = df[date_column][-1]
